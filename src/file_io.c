...
{	sf_count_t new_position ;
	...
	liDistanceToMove.QuadPart = (LONGLONG) offset ;

	fResult = SetFilePointerEx (psf->file.handle, liDistanceToMove,
		&liNewFilePointer, dwMoveMethod) ;
	...
	new_position = (sf_count_t) liNewFilePointer.QuadPart - psf->fileoffset ;
...
{	sf_count_t pos ;
	...
	pos = (LONGLONG) liNewFilePointer.QuadPart ;
...
{	int retval = 0 ;
	...
	*/
	...
	liDistanceToMove.QuadPart = (LONGLONG) len ;

	fResult = SetFilePointerEx (psf->file.handle, liDistanceToMove, NULL,
		FILE_BEGIN) ;
